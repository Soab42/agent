...
function streamLogs(task, socket) {
    ...
    const { site_name, source } = payload; // source: 'app' | 'nginx'

    let proc;
    if (source === 'app') {
        proc = spawn('journalctl', ['-u', `${site_name}.service`, '-f', '-n', '200', '--output=cat'], { shell: false });
    } else {
        const logFile = `/var/log/nginx/${site_name}.access.log`;
    ...
    // Stop streaming when socket disconnects
    socket.on('disconnect', () => proc.kill());
    socket.on('stop:logs', () => proc.kill());
